...
Sei $\D  \coloneqq  \CC_0^\infty(\R^d)$ die Menge (auch ein Vektorraum) der beliebig oft differenzierbaren Funktionen auf $\R^d$ mit beschränktem Träger. Etwa:
...
ist $\CC_0^\infty(\R^d)$ und hat kompakten Träger.
...
Die Zweite Funktion ist nicht in $\Ell^1$ da ihr Gesamtintegral nicht endlich ist jedoch ist das Integral über jede endlich Menge endlich, sie hat also keine Pole, somit sie in $\Ell^1_{loc}$.
...
Nun zu den Abbleitungen, sein zunächst $d=1$ und $f \in \CC^1 \subset \Ell^1_{\loc}$, dann gilt für alle $\varphi \in \D$ wobei $[-a,a] \supset supp(\varphi)$:
